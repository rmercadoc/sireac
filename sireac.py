...
def main(argv):
    ...
    try:
        ...
        print('sireac.py -s <path-to-score> --anger= --fear= --joy= --love= --sadness= --surprise=')
        sys.exit(2)
    if not opts:
        print(_help_message)
        sys.exit()
    ...
    if verbose:
        ...
        for recommendation in recommendations.chords:
            if recommendation.name is not None:
                log('{:>8} - {:<8}'.format(recommendation.degree, recommendation.name), center=' ')
            else:
                log('{:^8}'.format(recommendation.degree), center=' ')
    else:
